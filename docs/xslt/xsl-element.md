...
При создании элемента `<date>` используется функция [`current-date()`](../xpath/current-date.md). Чтобы увидеть, что произойдет в случае неудачной проверки, удалите элемент `<date>`. Появится следующее сообщение об ошибке:
