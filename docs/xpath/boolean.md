...
- Если аргумент является синглетом любого числового типа, `boolean()` возвращает `false`, если число является нулем или `NaN` («нечисло»); во всех остальных случаях возвращается `true`.
